...
class LogitM4CClassifier(nn.Module):
    def __init__(self, in_dim, out_dim, **kwargs):
        ...
        num_ans_candidates = out_dim
        text_non_linear_dim = kwargs["text_hidden_dim"]
        obj_non_linear_dim = kwargs["obj_hidden_dim"]
        ocr_non_linear_dim = kwargs["ocr_hidden_dim"]

        self.f_o_text = ReLUWithWeightNormFC(input_dim, text_non_linear_dim)
        self.f_o_obj = ReLUWithWeightNormFC(input_dim, obj_non_linear_dim)
        self.f_o_ocr = ReLUWithWeightNormFC(input_dim, ocr_non_linear_dim)
        self.linear_text = nn.Linear(text_non_linear_dim, num_ans_candidates)
        self.linear_obj = nn.Linear(obj_non_linear_dim, num_ans_candidates)
        self.linear_ocr = nn.Linear(ocr_non_linear_dim, num_ans_candidates)

        if "pretrained_text" in kwargs and kwargs["pretrained_text"] is not None:
            self.linear_text.weight.data.copy_(
                torch.from_numpy(kwargs["pretrained_text"])
            )

        if "pretrained_obj" in kwargs and kwargs["pretrained_obj"] is not None:
            self.linear_obj.weight.data.copy_(
                torch.from_numpy(kwargs["pretrained_obj"])
            )

        if "pretrained_ocr" in kwargs and kwargs["pretrained_ocr"] is not None:
            self.linear_ocr.weight.data.copy_(
                torch.from_numpy(kwargs["pretrained_ocr"])
            )

    def forward(self, joint_embedding):
        text_val = self.linear_text(self.f_o_text(joint_embedding))
        obj_val = self.linear_obj(self.f_o_obj(joint_embedding))
        ocr_val = self.linear_ocr(self.f_o_ocr(joint_embedding))
